...
def get_icp_transform(world_points, iterations):
    ...
    #print(world_points)
    o_trafo=(1.0, 1.0, 0.0, 0.0, 0.0)
    for i in range(iterations):
        nworld_points = [apply_transform(o_trafo, p) for p in world_points]
        left, right = get_corresponding_points_on_wall(nworld_points)
        trafo = estimate_transform(left, right, fix_scale = True)
        if trafo:
          o_trafo=concatenate_transform(trafo,o_trafo)
